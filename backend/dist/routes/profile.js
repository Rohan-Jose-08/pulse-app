...
router.put('/', authenticateUser, async (req, res) => {
    try {
        const { displayName, bio, profileImageUrl, location, website, socialLinks, interests, phoneNumber, dateOfBirth, gender, occupation, company, education, languages, timezone, preferences, placeId, latitude, longitude, accuracyMeters } = req.body;
        // Validate input
        ...
        if (languages && (!Array.isArray(languages) || languages.some((lang) => typeof lang !== 'string'))) {
        ...
        try {
            ...
            console.warn('Profile location update failed', locErr);
        }
...
router.get('/nearby', async (req, res) => {
    try {
        ...
        }).filter(Boolean).sort((a, b) => a.distanceKm - b.distanceKm).slice(0, 200);
