...
router.get('/conversations/:id/messages', authenticateUser, async (req, res) => {
    try {
        ...
        let legacy = await prisma.conversation.findUnique({ where: { id }, include: { participants: true } });
        if (!legacy) {
            // Try direct conversation id
            const dconv = await prisma.directConversation.findUnique({ where: { id }, include: { participants: { select: { id: true } } } }).catch(() => null);
            if (dconv) {
                legacy = await prisma.conversation.findUnique({ where: { id }, include: { participants: true } });
                if (!legacy) {
                    legacy = await prisma.conversation.create({
                        data: {
                            id,
                            isGroup: false,
        ...
        if (!legacy) {
            ...
            const pconv = await prisma.pulseConversation.findUnique({ where: { id }, include: { participants: { select: { id: true } }, pulse: { select: { id: true, title: true, imageUrl: true } } } }).catch(() => null);
            if (pconv) {
                let c = await prisma.conversation.findUnique({ where: { id }, include: { participants: true } });
                if (!c && pconv.pulse?.id) {
                    c = await prisma.conversation.findFirst({ where: { pulseId: pconv.pulse.id }, include: { participants: true } });
                }
                if (!c && pconv.pulse?.id) {
                ...
                if (pulse) {
                    const c = await prisma.conversation.findFirst({ where: { pulseId: id }, include: { participants: true } });
                    if (c)
                        legacy = c;
                }
            }
        ...
        const isParticipant = legacy.participants.some((p) => p.id === me);
        if (!isParticipant)
        ...
        res.json({
            messages: messages.reverse().map(m => ({
                ...
                senderPhotoUrl: m.sender?.profileImageUrl || null,
                reactions: reactionMap[m.id] || {},
                deliveredTo: m.deliveredTo || [],
                readBy: m.readBy || [],
                repliedTo: m.repliedTo ? {
                    id: m.repliedTo.id,
                    text: m.repliedTo.text,
                    imageUrl: m.repliedTo.imageUrl,
                    videoUrl: m.repliedTo.videoUrl,
                    senderId: m.repliedTo.senderId,
                    senderName: m.repliedTo.sender?.displayName || 'Unknown',
                } : null,
            })),
...
router.post('/direct-conversations/with/:otherUserId', authenticateUser, async (req, res) => {
    try {
        ...
        try {
            const legacy = await prisma.conversation.findUnique({ where: { id: convo.id } });
            if (!legacy) {
            ...
            console.warn('legacy conversation ensure failed (direct)', e);
        }
...
router.post('/pulse-conversations/by-pulse/:pulseId', authenticateUser, async (req, res) => {
    try {
        ...
        const me = req.user.id;
        if (!pulseId)
            return res.status(400).json({ error: 'pulseId required' });
        let convo = await prisma.pulseConversation.findUnique({
            where: { pulseId },
            include: {
                participants: { select: { id: true, displayName: true, profileImageUrl: true } },
                pulse: { select: { id: true, title: true, imageUrl: true } },
        ...
        if (!convo) {
            ...
            convo = await prisma.pulseConversation.create({
                data: {
            ...
            try {
                ...
                console.warn('legacy conversation ensure failed (pulse create)', e);
            }
        }
        else {
            ...
            if (!isParticipant) {
                ...
                const updated = await prisma.pulseConversation.findUnique({
                    where: { id: convo.id },
            ...
            try {
                ...
                console.warn('legacy conversation ensure failed (pulse fetch)', e);
            }
        }
        const mapped = {
            id: convo.id,
...
router.get('/pulse-conversations', authenticateUser, async (req, res) => {
    try {
        ...
        let convos = [];
        if (pulseIds.length) {
            convos = await prisma.pulseConversation.findMany({
        ...
        const byPulseId = new Map(convos.map((c) => [c.pulseId, c]));
        ...
        for (const pid of toEnsureCreate) {
            try {
                const created = await prisma.pulseConversation.create({
                ...
                try {
                    const legacy = await prisma.conversation.findUnique({ where: { id: created.id } });
                    if (!legacy) {
                        await prisma.conversation.create({
                            data: {
                                id: created.id,
        ...
        for (const c of convos) {
            const isParticipant = c.participants.some((p) => p.id === me);
            if (!isParticipant) {
                try {
                    await prisma.pulseConversation.update({
                        where: { id: c.id },
...
router.post('/group-conversations', authenticateUser, async (req, res) => {
    try {
        const me = req.user.id;
        const { name, description, avatarUrl, initialParticipantIds } = req.body;
        if (!name || name.trim().length === 0) {
            return res.status(400).json({ error: 'Group name is required' });
        }
        // Prepare participants: creator + initial members (deduplicated)
        const participantIds = new Set([me]);
        ...
        const convo = await prisma.groupConversation.create({
            data: {
                name: name.trim(),
...
router.get('/group-conversations/:id', authenticateUser, async (req, res) => {
    try {
        const me = req.user.id;
        const { id } = req.params;
        const convo = await prisma.groupConversation.findUnique({
            where: { id },
            include: {
                participants: { select: { id: true, displayName: true, profileImageUrl: true } },
                creator: { select: { id: true, displayName: true } }
...
router.get('/group-conversations', authenticateUser, async (req, res) => {
    try {
        const me = req.user.id;
        const convos = await prisma.groupConversation.findMany({
            where: { participants: { some: { id: me } } },
            orderBy: { updatedAt: 'desc' },
            include: {
                participants: { select: { id: true, displayName: true, profileImageUrl: true } },
...
router.get('/direct-conversations/:id/messages', authenticateUser, async (req, res) => {
    try {
        ...
        const { cursor, limit = '30' } = req.query;
        const take = Math.max(1, Math.min(parseInt(limit, 10) || 30, 100));
        const convo = await prisma.directConversation.findUnique({ where: { id }, include: { participants: true } });
        if (!convo)
            return res.status(404).json({ error: 'Conversation not found' });
        const isParticipant = convo.participants.some((p) => p.id === me);
        if (!isParticipant)
...
router.get('/pulse-conversations/:id/messages', authenticateUser, async (req, res) => {
    try {
        ...
        const { cursor, limit = '30' } = req.query;
        const take = Math.max(1, Math.min(parseInt(limit, 10) || 30, 100));
        const pconv = await prisma.pulseConversation.findUnique({ where: { id }, include: { participants: true, pulse: { select: { id: true } } } });
        if (!pconv)
            return res.status(404).json({ error: 'Conversation not found' });
        const isParticipant = pconv.participants.some((p) => p.id === me);
        if (!isParticipant)
            return res.status(403).json({ error: 'Forbidden' });
        // Resolve legacy Conversation id: prefer same id, else find by pulseId
        let legacyId = id;
        const legacy = await prisma.conversation.findUnique({ where: { id } });
        if (!legacy && pconv.pulse?.id) {
            const byPulse = await prisma.conversation.findFirst({ where: { pulseId: pconv.pulse.id } });
            if (byPulse)
                legacyId = byPulse.id;
        }
        const messages = await prisma.message.findMany({
...
router.post('/conversations/:id/messages', authenticateUser, async (req, res) => {
    try {
        ...
        if (!convo) {
            const pulse = await prisma.pulse.findUnique({ where: { id } });
            if (pulse) {
                convo = await prisma.conversation.findFirst({ where: { pulseId: id }, include: { participants: true, pulse: { select: { id: true } } } });
            }
        }
        if (!convo)
            return res.status(404).json({ error: 'Conversation not found' });
        // Ensure caller is participant; if pulse chat, auto-join if user is a pulse member
        let isParticipant = convo.participants.some((p) => p.id === me);
        if (!isParticipant && convo.pulse?.id) {
            try {
                ...
                const isPulseMember = !!pulse && (pulse.author.id === me || pulse.participants.some((p) => p.id === me));
                if (isPulseMember) {
        ...
        try {
            await prisma.directConversation.update({ where: { id: convo.id }, data: { updatedAt: new Date(), lastMessageText: text ?? (videoUrl ? '[video]' : (imageUrl ? '[image]' : '')), lastSenderId: me } }).catch(() => null);
            await prisma.pulseConversation.update({ where: { id: convo.id }, data: { updatedAt: new Date(), lastMessageText: text ?? (videoUrl ? '[video]' : (imageUrl ? '[image]' : '')), lastSenderId: me } }).catch(() => null);
        }
        catch (_) { }
...
router.post('/pulse-conversations/:id/messages', authenticateUser, async (req, res) => {
    try {
        ...
        const pconv = await prisma.pulseConversation.findUnique({ where: { id }, include: { participants: true, pulse: { select: { id: true, title: true, imageUrl: true } } } });
        if (!pconv)
            return res.status(404).json({ error: 'Conversation not found' });
        const isParticipant = pconv.participants.some((p) => p.id === me);
        if (!isParticipant)
        ...
        let legacy = await prisma.conversation.findUnique({ where: { id } });
        if (!legacy && pconv.pulse?.id) {
            legacy = await prisma.conversation.findFirst({ where: { pulseId: pconv.pulse.id } });
        }
        if (!legacy) {
        ...
        const msg = await prisma.message.create({ data: { conversationId: legacy.id, senderId: me, text: text ?? null, imageUrl: imageUrl ?? null, videoUrl: videoUrl ?? null } });
        await prisma.conversation.update({ where: { id: legacy.id }, data: { updatedAt: new Date(), lastMessageText: text ?? (videoUrl ? '[video]' : (imageUrl ? '[image]' : '')), lastSenderId: me } });
        await prisma.pulseConversation.update({ where: { id: pconv.id }, data: { updatedAt: new Date(), lastMessageText: text ?? (videoUrl ? '[video]' : (imageUrl ? '[image]' : '')), lastSenderId: me } }).catch(() => null);
        try {
            const io = (0, realtime_1.getIo)();
...
router.post('/direct-conversations/:id/messages', authenticateUser, async (req, res) => {
    try {
        ...
        if (!text && !imageUrl && !videoUrl)
            return res.status(400).json({ error: 'text or media required' });
        const dconv = await prisma.directConversation.findUnique({ where: { id }, include: { participants: true } });
        if (!dconv)
            return res.status(404).json({ error: 'Conversation not found' });
        const isParticipant = dconv.participants.some((p) => p.id === me);
        if (!isParticipant)
            return res.status(403).json({ error: 'Forbidden' });
        ...
        const msg = await prisma.message.create({ data: { conversationId: legacy.id, senderId: me, text: text ?? null, imageUrl: imageUrl ?? null, videoUrl: videoUrl ?? null } });
        await prisma.conversation.update({ where: { id: legacy.id }, data: { updatedAt: new Date(), lastMessageText: text ?? (videoUrl ? '[video]' : (imageUrl ? '[image]' : '')), lastSenderId: me } });
        await prisma.directConversation.update({ where: { id }, data: { updatedAt: new Date(), lastMessageText: text ?? (videoUrl ? '[video]' : (imageUrl ? '[image]' : '')), lastSenderId: me } }).catch(() => null);
        try {
            const io = (0, realtime_1.getIo)();
            io?.to(`conversation:${legacy.id}`).emit('message:new', { id: msg.id, conversationId: legacy.id, senderId: me, text: msg.text, imageUrl: msg.imageUrl, createdAt: msg.createdAt, videoUrl: msg.videoUrl });
            dconv.participants.forEach((p) => {
                const sockets = realtime_1.userSockets.get(p.id);
                if (!sockets)
                    return;
                sockets.forEach((sid) => {
        ...
        if (!legacy) {
            legacy = await prisma.conversation.create({
                data: {
                    ...
                    isGroup: true,
                    participants: {
                        connect: convo.participants.map((u) => ({ id: u.id }))
                    },
                    name: convo.name,
                    avatarUrl: convo.avatarUrl || null,
                },
