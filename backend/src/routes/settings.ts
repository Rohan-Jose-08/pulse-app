...
router.put('/', authenticateUser, async (req, res) => {
  try {
    const { 
      ...
      notifyNewPulsesNearby,
      notifyPulseInvitations,
      notifyPulseUpdates,
      notifyNewMessages,
      notifyNewFollowers,
    ...
    if (pulseHistoryVisibility !== undefined) updateData.pulseHistoryVisibility = pulseHistoryVisibility;
    if (friendRequestsFrom !== undefined) updateData.friendRequestsFrom = friendRequestsFrom;
    if (showOnlineStatus !== undefined) updateData.showOnlineStatus = showOnlineStatus;
    if (allowMessageRequests !== undefined) updateData.allowMessageRequests = allowMessageRequests;
    if (showInSearch !== undefined) updateData.showInSearch = showInSearch;
    
    // Notification settings
    if (pushNotificationsEnabled !== undefined) updateData.pushNotificationsEnabled = pushNotificationsEnabled;
    if (notifyNewPulsesNearby !== undefined) updateData.notifyNewPulsesNearby = notifyNewPulsesNearby;
    if (notifyPulseInvitations !== undefined) updateData.notifyPulseInvitations = notifyPulseInvitations;
    if (notifyPulseUpdates !== undefined) updateData.notifyPulseUpdates = notifyPulseUpdates;
    if (notifyNewMessages !== undefined) updateData.notifyNewMessages = notifyNewMessages;
    if (notifyNewFollowers !== undefined) updateData.notifyNewFollowers = notifyNewFollowers;
    if (notifyFriendRequests !== undefined) updateData.notifyFriendRequests = notifyFriendRequests;
    if (notifyPostReactions !== undefined) updateData.notifyPostReactions = notifyPostReactions;
    ...
    if (activityStatusVisible !== undefined) updateData.activityStatusVisible = activityStatusVisible;
